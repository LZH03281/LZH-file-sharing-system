...
- 后续可在 Ubuntu Server 上接入 ClamAV，对上传文件进行入库前扫描。
...
后续安全加固建议优先考虑 **ClamAV**。它是开源杀毒引擎，适合部署在 Ubuntu Server 上，与本项目“服务端统一处理文件安全”的结构比较匹配。

推荐接入点：

```text
上传文件 → 写入 storage/.tmp → ClamAV 扫描
        → 扫描通过：移动到正式存储目录，写入 files 表
        → 扫描失败：删除临时文件，返回 FILE_INFECTED/SCAN_FAILED，写入 operation_logs
```

建议实现方式：

- Ubuntu Server 安装 `clamav`、`clamav-daemon`，定期更新病毒库；
- 服务端新增 `VirusScanService`，封装 `clamd` 或 `clamscan` 调用；
- 先采用同步扫描，保持课程项目逻辑简单；
- 扫描结果写入操作日志，方便管理员导出和排查；
- ClamAV 服务不可用时采用保守策略：拒绝上传并提示管理员检查服务；
- 不在客户端直接杀毒，客户端只展示服务端返回的安全提示。

该方向属于后续增强，当前代码主干暂未实现杀毒扫描。
