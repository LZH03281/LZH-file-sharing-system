...
后续增强：操作日志与管理员界面、传输进度细化、Ubuntu Server 常驻部署、密码哈希方案升级，以及基于 ClamAV 的上传文件安全扫描。

## 当前进度

当前已完成项目基础版：FastAPI 服务端、SQLite/JWT 权限、PyQt6 客户端主界面、操作日志和 Ubuntu 部署说明。服务端测试结果为 `11 passed`，客户端已通过语法检查和窗口构造检查。
...
下一步可以考虑在 Ubuntu Server 上接入 **ClamAV** 开源杀毒引擎，用于上传文件的基础病毒扫描。建议由服务端统一调用 `clamd` 或 `clamscan`，在文件写入临时目录后、正式入库前完成扫描：

- 扫描通过：继续保存文件元数据并允许下载；
- 扫描失败或发现病毒：删除临时文件，返回明确错误提示，并写入操作日志；
- 服务不可用：优先采用保守策略，拒绝本次上传或提示管理员检查杀毒服务。

该功能暂未实现，适合作为后续同学接手的安全加固任务。
